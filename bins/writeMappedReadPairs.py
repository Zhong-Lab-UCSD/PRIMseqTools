...
dicReadIdStrand1=defaultdict(list)
dicReadIdStrand2=defaultdict(list)
...
with open('%s/%salignment/read1_tx/mapped.sorted.bed_chunk%s'%(sys.argv[1],sys.argv[5],sys.argv[2]),'r') as f:
    for line in f:
        ...
        dicReadIdStrand1[splitLine[3]].append(splitLine[5])
...
with open('%s/%salignment/read2_tx/mapped.sorted.bed_chunk%s'%(sys.argv[1],sys.argv[5],sys.argv[2]),'r') as f:
    for line in f:
        ...
        if dicRead1_count[readId]>0:
            ...
            dicReadIdStrand2[splitLine[3]].append(splitLine[5])

idList=list(set(idList))

targetFile1=open('%s/%sintermediateFiles/mappedReadPairs_all_bwa.csv_%s'%(sys.argv[1],sys.argv[5],sys.argv[2]),'w')
targetFile2=open('%s/%sintermediateFiles/chimericReadPairs_all_bwa.csv_%s'%(sys.argv[1],sys.argv[5],sys.argv[2]),'w')
targetFile2.write('readId,R1Tx,R1start,R1end,R1Gene,R1Cigar,R1Strand,R2Tx,R2start,R2end,R2Gene,R2Cigar,R2Strand\n')
for readId in idList:
    ...
    strand1,strand2=dicReadIdStrand1[readId][0],dicReadIdStrand2[readId][0]
    ...
    if len(set(geneList1)&set(geneList2))==0:
        ...
        if flag1 and flag2:
            gene1,gene2=dicIdGeneName[txId1],dicIdGeneName[txId2]
            #write file
            targetFile2.write('%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n'
                                 %(readId,txId1,start1,end1,gene1,str(cigar1),strand1,txId2,start2,end2,gene2,str(cigar2),strand2))
